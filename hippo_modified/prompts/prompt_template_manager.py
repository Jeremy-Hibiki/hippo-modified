...
class PromptTemplateManager:
    ...
    templates: dict[str, Template | list[dict[str, Any]]] = field(
        init=False,
    ...
    def _load_templates(self) -> None:
        ...
        for filename in Path.iterdir(self.templates_dir):
            if filename.suffix == ".py" and filename.name != "__init__.py":
                ...
                try:
                    try:
                        module_name = f"hippo_modified.prompts.templates.{script_name}"
                        module = importlib.import_module(module_name)
                    except ModuleNotFoundError:
                        module_name = f".prompts.templates.{script_name}"
                        module = importlib.import_module(module_name, "hippo_modified")
    ...
    def render(self, name: str, **kwargs) -> str | list[dict[str, Any]]:
        ...
        if isinstance(template, Template):
            ...
            try:
                ...
                raise ValueError(f"Missing variable for template '{name}': {e}") from None
    ...
    def get_template(self, name: str) -> Template | list[dict[str, Any]]:
        """
