...
class VLLMOfflineOpenIE(OpenIE):
    ...
    def batch_openie(self, chunks: dict[str, ChunkInfo]) -> tuple[dict[str, NerRawOutput], dict[str, TripleRawOutput]]:
        ...
        triple_extract_input_messages = [
            ...
            for passage, named_entities in zip(chunk_passages.values(), ner_output, strict=False)
        ]
        ...
        for idx, ner_output_instance in enumerate(ner_output):
            ...
            if len(unique_entities) == 0:
                logger.warning(f"No entities extracted for chunk_id: {chunk_id}")
        ...
        ner_results_dict = {
            ...
            for chunk_key, ner_raw_output in zip(chunks.keys(), ner_raw_outputs, strict=False)
        }
        ...
        for idx, triple_output_instance in enumerate(triple_output):
            ...
            try:
                ...
                logger.warning(f"Could not parse response from OpenIE: {e}")
            if len(triples) == 0:
                logger.warning(f"No triples extracted for chunk_id: {chunk_id}")
        ...
        triple_results_dict = {
            ...
            for chunk_key, triple_raw_output in zip(chunks.keys(), triple_raw_outputs, strict=False)
        }
