...
class VLLMOffline:
    ...
    def __init__(self, global_config, cache_dir=None, cache_filename=None, max_model_len=4096, **kwargs):
        try:
            ...
            raise ImportError("vllm is not installed, please install it with `pip install vllm`") from None
    ...
    def infer(self, messages: list[TextChatMessage], max_tokens=2048):
        ...
        vllm_output = self.client.generate(
            prompt_token_ids=prompt_ids, sampling_params=SamplingParams(max_tokens=max_tokens, temperature=0)
        )
    ...
    def batch_infer(self, messages_list: list[list[TextChatMessage]], max_tokens=2048, json_template=None):
        ...
        all_prompt_ids = [
            convert_text_chat_messages_to_input_ids(messages, self.tokenizer) for messages in messages_list
        ]
        vllm_output = self.client.generate(
