...
class HippoRAG(BaseHippoRAG, HippoRAGProtocol):
    def __init__(
        self,
    ...
    def index(self, docs: list[DocumentInput]):
        """
        ...
        Parameters:
            docs : List[Union[str, dict]]
                ...
                Example: [
                    "Plain text document",
                    {"content": "Document with metadata", "metadata": {"source": "doc1", "category": "tech"}}
                ]
        """
        ...
        for doc in docs:
            if isinstance(doc, str):
                ...
                if "content" not in doc:
                    raise ValueError(f"Document dict must have 'content' key, got: {list(doc.keys())}")
        ...
        # Pass to embedding store with metadata
        self.chunk_embedding_store.insert_strings(normalized_docs, metadatas)
    ...
    def retrieve(
        ...
        for q_idx, query in enumerate(queries):
            ...
            if metadata_filters:
                # Filter chunk_ids based on metadata
                filtered_chunk_ids = [
            ...
            if chunk_ids_to_retrieve:
                top_k_docs = [
                    v["content"] for v in (self.chunk_embedding_store.get_rows(chunk_ids_to_retrieve)).values()
                ]
            else:
                top_k_docs = []
    ...
    def cleanup_invalid_pike_mappings(self) -> None:
        ...
        if mappings_to_update:
            self.query_embedding_store.upsert_query_mappings(mappings_to_update)
