...
class EmbeddingStore:
    ...
    def get_missing_string_hash_ids(self, texts: list[str]):
        ...
        return {h: {"hash_id": h, "content": t} for h, t in zip(missing_ids, texts_to_encode, strict=False)}
    ...
    def _load_data(self):
        if os.path.exists(self.filename):
            ...
            self.hash_id_to_row = {h: {"hash_id": h, "content": t} for h, t in zip(self.hash_ids, self.texts, strict=False)}
    ...
    def _save_data(self):
        ...
        self.hash_id_to_row = {
            h: {"hash_id": h, "content": t} for h, t, e in zip(self.hash_ids, self.texts, self.embeddings, strict=False)
        }
        self.hash_id_to_idx = {h: idx for idx, h in enumerate(self.hash_ids)}
        self.hash_id_to_text = {h: self.texts[idx] for idx, h in enumerate(self.hash_ids)}
        self.text_to_hash_id = {self.texts[idx]: h for idx, h in enumerate(self.hash_ids)}
