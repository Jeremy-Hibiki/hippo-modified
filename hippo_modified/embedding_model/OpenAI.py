...
class OpenAIEmbeddingModel(BaseEmbeddingModel):
    def __init__(self, global_config: BaseConfig | None = None, embedding_model_name: str | None = None) -> None:
        ...
        if self.global_config.azure_embedding_endpoint is None:
            ...
            self.async_client = AsyncOpenAI(base_url=self.global_config.embedding_base_url)
        else:
    ...
    async def async_batch_encode(self, texts: list[str], **kwargs) -> npt.NDArray:
        ...
        if len(texts) <= batch_size:
            results = await self.async_encode(texts)
        else:
            with tqdm(total=len(texts), desc="Batch Encoding") as pbar:
                results = []
                for i in range(0, len(texts), batch_size):
                    batch = texts[i : i + batch_size]
                    results.append(await self.async_encode(batch))
                    pbar.update(batch_size)
            results = np.concatenate(results)
