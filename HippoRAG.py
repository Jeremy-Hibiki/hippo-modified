...
from llama_index_exts_fmsh.indices.raptor_tree.base import RaptorGetNodeMode, RaptorNodeArranger, RaptorTreeIndex
...
from llama_index.core import ChatPromptTemplate, PromptHelper
from transformers import AutoTokenizer
from llama_index.postprocessor.jinaai_rerank.base import JinaRerank
from llama_index.core.chat_engine.types import StreamingAgentChatResponse
from llama_index.core.chat_engine.types import StreamingAgentChatResponse, StreamingResponse
from openai import Stream
from phoenix.otel import register
from openinference.instrumentation.llama_index import LlamaIndexInstrumentor
from openinference.instrumentation.openai import OpenAIInstrumentor
from hipporag.milvus import MilvusWithBM25Function, TritonRemoteM3SparseEmbeddingFunction
...
tracer_provider = register(protocol="http/protobuf", project_name="hippo_test", endpoint="http://192.168.131.115:10306/v1/traces")
...
class HippoRAG:
    ...
    def __init__(self,
        ...
        if rerank_api_url  is not None:
            self.rerank_api_url = rerank_api_url
        else:
            self.rerank_api_url = "http://192.168.131.119:7997"

        if rerank_model  is not None:
            self.rerank_model = rerank_model
        else:
            self.rerank_model = "jinaai/jina-reranker-v2-base-multilingual"
        ...
        if rerank_threshold  is not None:
            ...
            self.rerank_threshold = 0.1
            
        if retrieve_num is not None:
            self.retrieve_num = retrieve_num
        else:
            self.retrieve_num = 40
        ...
        # rerank model
        # self.reranker = TextEmbeddingsInferenceRerank(
        #     base_url=self.rerank_api_url,
    ...
    def index(self, docs: List[TextNode]):
        """
        ...
        """
        docs = [node.text for node in docs]
        logger.info(f"Indexing Documents")
    ...
    def retrieve(self,
                 ...
                 rerank_batch_num: int=10) -> List[QuerySolution] | Tuple[List[QuerySolution], Dict]:
        """
        ...
        for q_idx, query in tqdm(enumerate(queries), desc="Retrieving", total=len(queries)):
            ...
            top_k_fact_indices, top_k_facts, rerank_log = self.rerank_facts(query, query_fact_scores, batch_num=rerank_batch_num)
            rerank_end = time.time()

            self.rerank_time += rerank_end - rerank_start
            #top_k_facts = [] # dpr force
            if len(top_k_facts) == 0 or debug:
                logger.info('No facts found after reranking, return DPR results')
                sorted_doc_ids, sorted_doc_scores = self.dense_passage_retrieval(query)
                if do_rerank:
                    sorted_doc_ids, sorted_doc_scores = self.rerank_passage(query, sorted_doc_ids, sorted_doc_scores)
                if self.use_tree:
    ...
    def rag_qa(self,
               ...
               qa_top_k: int = None,
               temperature: float = 0.0) -> Tuple[List[QuerySolution], List[str], List[Dict]] | Tuple[List[QuerySolution], List[str], List[Dict], Dict, Dict]:
        """
        ...
        if isinstance(queries[0], QuerySolution):# and len(queries) == 1:
            all_response = []
            queries_solutions, all_qa_messages = self.stream_qa(queries, temperature=temperature)
            for i, stream in enumerate(queries_solutions):
                source_nodes = all_qa_messages[i]
                response = self.answer(stream, source_nodes)
                all_response.append(response)
            return all_response
        
        # # Performing QA
        # queries_solutions, all_response_message, all_metadata = self.qa(queries, temperature=temperature)
    ...
    def qa(self, queries: List[QuerySolution], temperature: float=0.0) -> Tuple[List[QuerySolution], List[str], List[Dict]]:
        """
        ...
        all_qa_results = [self.llm_model.infer(qa_messages, temperature=temperature) for qa_messages in tqdm(all_qa_messages, desc="QA Reading")]
    ...
    #@tracer.chain
    def stream_qa(self, queries: List[QuerySolution], temperature: float=0.0) -> Tuple[List[QuerySolution], List[str], List[Dict]]:
        """
        Executes question-answering (QA) inference using a provided set of query solutions and a language model.
        ...
        for query_solution in tqdm(queries, desc="Collecting QA prompts"):
            if len(query_solution.docs) == 0:
                #print(query_solution.question)
                response = self.tree_qa(query_solution, temperature=temperature)
                queries_solutions.append(response)
                all_qa_messages.append([response.source_nodes])
            else:
                # obtain the retrieved docs
                source_nodes = []
                retrieved_passages = query_solution.docs[:self.global_config.qa_top_k]
                ...
                if self.prompt_template_manager.is_template_name_valid(name=f'rag_qa_{self.global_config.dataset}'):
                    ...
                    logger.debug(
                        f"rag_qa_{self.global_config.dataset} does not have a customized prompt template. Using MUSIQUE's prompt template instead.")
                    prompt_dataset_name = 'musique'
                qa_message = self.prompt_template_manager.render(name=f'rag_qa_{prompt_dataset_name}', prompt_user=prompt_user)
    
                response, _, _ = self.llm_model.infer(qa_message, temperature=temperature, stream=True)
                queries_solutions.append(response)
                all_qa_messages.append(source_nodes)
        # all_qa_results = [self.llm_model.infer(qa_messages, temperature=temperature) for qa_messages in tqdm(all_qa_messages, desc="QA Reading")]
        ...
        # Process responses and extract predicted answers.
        # queries_solutions = [] # process streaming response
        # for query_solution_idx, query_solution in tqdm(enumerate(queries), desc="Extraction Answers from LLM Response"):
        #     response_content = all_response_message[query_solution_idx]
        #     #try:
        #     #    pred_ans = response_content.split('答案:')[1].strip()
        ...
        return queries_solutions, all_qa_messages
    ...
    def answer(self, response, source_nodes):
        if isinstance(response, Stream):
            for chunk in response:
                if chunk.choices and chunk.choices[0].delta.content:  # 检查是否有内容
                    s = StreamingAgentChatResponse(
    ...
    def get_vector_store(self,):
        ...
        store = MilvusWithBM25Function(
            uri=address,
            user= self.milvus_username,
            password=self.milvus_password,
            collection_name=collection,
            dim=self.embedding_dim, # 768
            enable_sparse=self.enable_sparse,
            sparse_embedding_function=TritonRemoteM3SparseEmbeddingFunction(
                self.tokenizer,
                server_url="192.168.131.116:8001",
                model_name="bge-m3",
            ),
            text_key="text",
            output_fields=["text", "_node_type", "_node_content"],
        )
        return store 

    def tree_qa(self, query:QuerySolution, temperature:float=0) -> Tuple[np.ndarray, np.ndarray]:
        print("Using chunk-size=2048 Tree Index")
        storage_context = StorageContext.from_defaults(persist_dir=self.persist_dir)
        # index_graph = storage_context.index_store.index_structs()[0]
        # docstore = storage_context.docstore
        # vetorindex init
        DEFAULT_EMBED_MODEL = TextEmbeddingsInference(
            # 需要 model_name 用于 Embedding 添加 instruction
            model_name="BAAI/bge-m3",
            base_url="http://192.168.131.116:8892",
            embed_batch_size=32,
            truncate_text=True,
        )
        ...
        # for idx in index_graph.all_nodes:
        #     node_id = index_graph.all_nodes[idx]
        ...
        #     api_key="fk-fake",
        #     api_base=self.global_config.embedding_base_url,
        ...
        vector_index = VectorStoreIndex.from_vector_store(store, embed_model=DEFAULT_EMBED_MODEL)

        DEFAULT_RERANKER = JinaRerank(
            model=self.rerank_model,
            base_url=self.rerank_api_url,
            top_n=self.rerank_top_k,
        )

        node_arranger = RaptorNodeArranger(storage_context=storage_context, remove_overlap=False)

        llm = OpenAILike(model=self.global_config.llm_name, api_base=self.global_config.llm_base_url,
                 api_key=os.getenv('OPENAI_API_KEY'), is_function_calling_model=False, 
                 is_chat_model=True, 
                 context_window=131072, max_tokens=8192, temperature=temperature)
        
        tokenizer = self.tokenizer #AutoTokenizer.from_pretrained(self.tokenizer_dir)
        ...
        response_synthesizer = RateLimitedTreeSummarize(
            ...
            prompt_helper=PromptHelper.from_llm_metadata(
                llm.metadata,
                tokenizer=tokenizer.encode,  # type: ignore
            ),
            verbose=True,
            streaming=True,
            )
        # retrieve
        # query_retriever = vector_index.as_retriever(similarity_top_k=40)
        # query = QueryBundle(query_str=query)
        # retrieve_nodes = query_retriever.retrieve(query)
        # rerank_nodes = DEFAULT_RERANKER.postprocess_nodes(retrieve_nodes, query)
        # thresh_nodes = SimilarityPostprocessor(similarity_cutoff=0.1).postprocess_nodes(rerank_nodes)
        # final_nodes = node_arranger.postprocess_nodes(thresh_nodes)
        ...
        #     text = node.text
        #     hash_id = self.chunk_embedding_store.get_hash_id(text)
        #     idx = self.passage_node_keys.index(hash_id)
        #     sorted_doc_ids.append(idx)
        #     sorted_doc_scores.append(node.score)
        # return np.array(sorted_doc_ids), sorted_doc_scores
    ...
    def rerank_passage(self, query, sorted_doc_ids, sorted_doc_scores):
        # do rerank here
        retrieve_nodes = []
        querybundle = QueryBundle(query)
        text_id_dict = {}
        for i, idx in enumerate(sorted_doc_ids[:self.retrieve_num]):
            doc_text = self.chunk_embedding_store.get_row(self.passage_node_keys[idx])["content"]
            text_id_dict[doc_text] = idx
            text_node = TextNode(text=doc_text)
            node_with_score = NodeWithScore(node=text_node, score=sorted_doc_scores[i])
            retrieve_nodes.append(node_with_score)
        
        nodes_after_rerank = self.reranker.postprocess_nodes(retrieve_nodes, querybundle)
        sorted_doc_ids = []
        sorted_doc_scores = []
        for node_reranked in nodes_after_rerank:
            score = node_reranked.score
            #score = 0
            doc = node_reranked.text
            origin_id = text_id_dict.get(doc, -1)
            sorted_doc_ids.append(origin_id)
    ...
    def graph_search_with_fact_entities(self, query: str,
                                        ...
                                        do_rerank: bool = False) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        if do_rerank:
            dpr_sorted_doc_ids, dpr_sorted_doc_scores = self.rerank_passage(query, dpr_sorted_doc_ids, dpr_sorted_doc_scores)
        ...
        #node_weights =  phrase_weights # dpr only
    ...
    def rerank_facts(self, query: str, query_fact_scores: np.ndarray, batch_num: int=10) -> Tuple[List[int], List[Tuple], dict]:
        """
        ...
        print(f"query: {query}")
        print(f"candidate_facts: {candidate_facts}")
        top_k_fact_indices, top_k_facts, reranker_dict = self.rerank_filter(query,
                                                                             candidate_facts,
                                                                             candidate_fact_indices,
                                                                             len_after_rerank=link_top_k,
                                                                             batch_num=batch_num)
        print(f"top_k_facts: {top_k_facts}")
